/**
 * Synthetic source, generated inside DuckDB. A demo fixture, not library surface — which is
 * why it is a `SourceProvider` the demo registers rather than something `Runtime` imports.
 *
 * The column types are chosen deliberately to exercise every upload tier, because a demo
 * where everything happens to be a clean single-batch Float32 column proves nothing about
 * the hard cases:
 *
 *   elevation, hour  FLOAT, no nulls  -> 'chunked' tier, no JS element loop
...
/**
 * A provider whose row count is read from the graph node at materialize time, so the demo's
 * row-count selector can change it without editing the graph JSON.
 */
export function syntheticSource(getRows: () => number, seed = 0.42): SourceProvider {
  return {
    relation: `"${SOURCE_TABLE}"`,
    describe: 'synthetic 9-cluster point cloud',
    async materialize(sql: SqlEngine) {
      await sql.resetPrepared();
      await sql.exec(syntheticSql(getRows(), seed));
    },
  };
}
